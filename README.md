...
flowchart LR
    ...
    style A fill:#e8f0fe,stroke:#1a73e8,color:#1a1a2e
    style C fill:#f3e8fd,stroke:#7b1fa2,color:#1a1a2e
    style D fill:#fef7e0,stroke:#e37400,color:#1a1a2e
    style E fill:#fef7e0,stroke:#e37400,color:#1a1a2e
    style F fill:#f3e8fd,stroke:#7b1fa2,color:#1a1a2e
    style G fill:#e6f4ea,stroke:#1e8e3e,color:#1a1a2e
    style H fill:#e6f4ea,stroke:#1e8e3e,color:#1a1a2e
    style I fill:#f1f3f4,stroke:#5f6368,color:#1a1a2e
```

> **每週流程**：上傳 PPTX → 自動轉換 → VLM 分類 → 人工確認 → VLM 提取 → 審核修正 → 存入資料庫 → 搜尋管理
...
flowchart TB
    ...
    subgraph Stage1["Stage 1 — VLM 分類"]
        CL1["VLM 逐頁判斷\n案例 or 非案例"]
        CL2["信心分數\n+ 分類結果"]
        CL1 --> CL2
    end
    ...
    subgraph Stage2["Stage 2 — VLM 結構化提取"]
        EX1["並行提取 10 個欄位\nstructured output"]
        EX2["資料清洗\n日期/數值格式化"]
        EX1 --> EX2
    end
...
> **Key rules**:
> - PPTX 原始檔在轉換為 PDF + PNG 後**自動刪除**，轉換後的 PDF 和所有頁面 PNG 永久保留。
> - VLM 提取結果**不直接寫入資料庫**——必須經過人工審核（Review UI）後才存入 `fa_cases`。
> - 案例的原始 VLM 回應保留在 `raw_vlm_response` 欄位，供後續分析 VLM 品質。
> - Embedding 在案例確認存入後**自動生成**（text + image），用於語義搜尋和相似案例推薦。
