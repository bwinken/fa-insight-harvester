...
| `~/opt/qvault/deploy/.env` | Docker 服務設定（PG 初始化、OIDC） | docker-compose |
...
# 編輯 deploy/.env — 填入：PG_PASSWORD, OIDC_ISSUER_URL, OAUTH2_CLIENT_SECRET, OAUTH2_REDIRECT_URL
...
├── deploy/
│   ├── .env                      # Docker 設定檔（PG + oauth2-proxy）
│   ├── docker-compose.yml        # PG + oauth2-proxy（讀取 deploy/.env）
