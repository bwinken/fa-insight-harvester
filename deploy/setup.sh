...
# Read DATA_DIR from deploy/.env (set during step 2)
source "$DEPLOY_DIR/.env"
DATA_DIR="${DATA_DIR:-/mnt/db/qvault}"
...
if [ ! -f "$APP_DIR/.env" ]; then
    ...
    sed -i \
        ...
        -e "s|UPLOAD_DIR=.*|UPLOAD_DIR=${DATA_DIR}/uploads|" \
        -e "s|LOG_DIR=.*|LOG_DIR=${DATA_DIR}/logs|" \
        -e "s|AUTH_PUBLIC_KEY_PATH=.*|AUTH_PUBLIC_KEY_PATH=${DATA_DIR}/keys/public.pem|" \
        "$APP_DIR/.env"
    echo ".env 已建立（已自動填入 DATABASE_URL + 資料路徑）✓"
...
echo "  2. 已放置 $DATA_DIR/keys/public.pem（Auth Center RS256 公鑰）"
